...
export function MeetingRoomBookings({ loggedInTenantId = null }: MeetingRoomBookingsProps = {}) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      {selectedView === 'list' && (
        <div className="bg-dark-900 rounded-lg shadow-lg border border-dark-700 overflow-hidden flex flex-col">
      ...
      {selectedView === 'invoiced' && (
        <div className="bg-dark-900 rounded-lg shadow-lg border border-dark-700 flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          {(() => {
            let invoicedBookingsForCount = allBookings
              .filter(b => b.invoice_id !== null)
              .filter(b => {
                if (selectedTenantFilter === 'all') return true;
                if (b.booking_type === 'tenant') return b.tenant_id === selectedTenantFilter;
            ...
            if (searchQuery.trim()) {
              const q = searchQuery.toLowerCase();
              invoicedBookingsForCount = invoicedBookingsForCount.filter(b => {
                const tenantName = b.tenants?.name?.toLowerCase() || '';
                const tenantCompany = b.tenants?.company_name?.toLowerCase() || '';
                const externalName = b.external_customers?.contact_name?.toLowerCase() || '';
                const externalCompany = b.external_customers?.company_name?.toLowerCase() || '';
            ...
            if (dateRange.from) {
              invoicedBookingsForCount = invoicedBookingsForCount.filter(b => b.booking_date >= dateRange.from);
            }
            if (dateRange.to) {
              invoicedBookingsForCount = invoicedBookingsForCount.filter(b => b.booking_date <= dateRange.to);
            }
            return (
              <Pagination
                currentPage={currentPage}
                totalItems={invoicedBookingsForCount.length}
                pageSize={pageSize}
                onPageChange={(page) => { setCurrentPage(page); }}
                onPageSizeChange={(size) => { setPageSize(size); setCurrentPage(1); }}
                label="boekingen"
